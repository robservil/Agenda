...
export default function CreateOrderScreen ({ navigation, route }) {
  ...
  const validationSchema = yup.object().shape({
    ...
    restaurantId: yup.number().positive().integer().min(1).required('Restaurant category is required'),
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (isValid) {
        ...
        showMessage({
          message: `Order with price ${createdOrder.price} (including shipping costs) succesfully created`,
          type: 'success',
          style: GlobalStyles.flashStyle,
          titleStyle: GlobalStyles.flashTextStyle
        })
      ...
      showMessage({
        message: `The order could not be created because of error ${error}.`,
        type: 'error',
        style: GlobalStyles.flashStyle,
        titleStyle: GlobalStyles.flashTextStyle
      })
  ...
  return (
    <ScrollView contentContainerStyle={styles.contentContainer}>
      ...
      <Pressable
        ...
        style={({ pressed }) => [
          {
            backgroundColor: pressed ? GlobalStyles.brandSecondaryTap : GlobalStyles.brandSecondary
          },
          styles.actionButton
        ]}>
        <View style={styles.buttonContent}>
          <MaterialCommunityIcons name='cancel' color={'white'} size={20}/>
          <TextRegular style={styles.text}>
            Cancel
          </TextRegular>
        </View>
